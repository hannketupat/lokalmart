...
<div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <h1 class="text-xl md:text-2xl lg:text-3xl font-bold text-gray-900">Notifikasi</h1>
            @if($unreadCount > 0)
                <form action="{{ route('notifications.readAll') }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <button type="submit"
                            class="w-full sm:w-auto px-4 py-2 text-sm font-medium text-indigo-600 hover:text-indigo-700 bg-indigo-50 rounded-lg border border-indigo-200 hover:bg-indigo-100 transition">
                        Tandai semua dibaca
                    </button>
                </form>
            @endif
    ...
    <div class="mb-6 border-b border-gray-200 overflow-x-auto">
        <nav class="flex gap-1 min-w-max" aria-label="Filter notifikasi">
            <a href="{{ route('notifications.index') }}"
               class="whitespace-nowrap px-4 py-2.5 text-sm font-medium rounded-t-lg {{ request()->query('type', 'all') === 'all' ? 'bg-indigo-50 text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50' }}">
            ...
            <a href="{{ route('notifications.index', ['type' => 'transaksi']) }}"
               class="whitespace-nowrap px-4 py-2.5 text-sm font-medium rounded-t-lg {{ request()->query('type') === 'transaksi' ? 'bg-indigo-50 text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50' }}">
            ...
            <a href="{{ route('notifications.index', ['type' => 'chat']) }}"
               class="whitespace-nowrap px-4 py-2.5 text-sm font-medium rounded-t-lg {{ request()->query('type') === 'chat' ? 'bg-indigo-50 text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50' }}">
            ...
            <a href="{{ route('notifications.index', ['type' => 'sistem']) }}"
               class="whitespace-nowrap px-4 py-2.5 text-sm font-medium rounded-t-lg {{ request()->query('type') === 'sistem' ? 'bg-indigo-50 text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50' }}">
